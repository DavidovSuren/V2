...
const DB_PATH = process.env.DB_PATH || path.join(__dirname, 'data', 'version20.sqlite');
...
-- скопированы из банка на момент построения плана), чтобы не зависеть от
-- банка заданий при каждом чтении и не терять текст, если банк изменится.
-- "Пропуск" НЕ двигает day_index — тот же день просто остаётся текущим
-- (users.day_index не увеличивается), поэтому отдельной даты на строку
-- не нужно: план не "сдвигается" физически, сдвигается только время
-- достижения 365/365 из-за пропущенных календарных дней.
