...
router.post('/diary', identify, requireUser, wrap(async (req, res) => {
  ...
  const scheduleRow = await db.get('SELECT * FROM user_schedule WHERE user_id = ? AND day_index = ?',
  ...
  await db.transaction(async (t) => {
    ...
    await t.run(`
      ...
      WHERE id = ?
    `, [completedCount, xpGain, level, newStreak, streakBest, today, user.id]);
  ...
  const unlockedCodes = await checkAndUnlock(updatedUser);
...
router.get('/diary', identify, requireUser, wrap(async (req, res) => {
