...
router.get('/progress', identify, requireUser, wrap(async (req, res) => {
  const user = req.user;
...
router.get('/achievements', identify, requireUser, wrap(async (req, res) => {
  const rows = await db.all('SELECT code, unlocked_at FROM achievements WHERE user_id = ?', [req.user.id]);
